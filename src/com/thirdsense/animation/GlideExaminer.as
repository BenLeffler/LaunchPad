package com.thirdsense.animation 
{
	...
	public class GlideExaminer 
	{
		...
		public function GlideExaminer( target:MovieClip, includeChildren:Boolean = true ) 
		{
		...
		private function convertToTexture(disp:DisplayObject, index:int, includeChildren:Boolean, temp_name:String ):void
		{
			...
			if ( !this.textures[temp_name] )
			{
				...
				var tp:TexturePack = new TexturePack( ss );
				this.textures[temp_name] = { texture_pack:tp, index:index, id:this.totalElements };
			}
			
			//this.target.addChildAt( disp, index );
		}
		...
		public final function getElementAt( id:int ):TexturePack
		{
			for ( var str:String in this.textures )
			{
				//if ( this.textures[str].index == id )
				if ( this.textures[str].id == id )
				{
					return this.textures[str].texture_pack;
				}
			}
