package com.thirdsense.animation 
{
	...
	public dynamic class GlideConstruct extends Sprite implements IAnimatable
	{
		...
		public function GlideConstruct( examiner:GlideExaminer ) 
		{
		...
		private function reconstruct():void
		{
			...
			{
				var tp:TexturePack = examiner.getElementAt(i);
				
				if ( tp.getTextures().length == 1 )
				{
				...
				{
					var mc:starling.display.MovieClip = tp.getMovieClip();
					mc.name = tp.sequence;
					this[mc.name] = mc;
					this.addChild( mc );
				}
		...
		{
			removeEventListener(Event.REMOVED_FROM_STAGE, examinerRemoveHandler);
